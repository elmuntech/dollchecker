...
| Deploy `analyze-toy`, `delete-account` | `supabase functions deploy <name>` | Scanning, account deletion | ⬜ |
...
## 2. Polar.sh (payments — planned)

| Item | Where it goes | Blocks | Status |
|------|---------------|--------|--------|
| Polar organization + product ("DollChecker Premium", monthly + yearly prices) | Polar dashboard | Paywall | ⬜ |
| `POLAR_ACCESS_TOKEN` (server-side, organization access token) | `supabase secrets set` | Checkout creation | ⬜ |
| `POLAR_WEBHOOK_SECRET` | `supabase secrets set` | Tier upgrades landing in the DB | ⬜ |
| Product / price IDs | `supabase secrets set POLAR_PRODUCT_ID=...` | Checkout creation | ⬜ |
| Webhook endpoint registered in Polar → `https://<project>.supabase.co/functions/v1/polar-webhook` | Polar dashboard | Tier upgrades | ⬜ |
| Sandbox vs production toggle (`POLAR_SERVER=sandbox\|production`) | `supabase secrets set` | Testing before launch | ⬜ |
| Success/return URL (`dollchecker://checkout-done`) | Polar product settings | Returning to the app after paying | ⬜ |
