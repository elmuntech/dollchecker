...
| ~~App icon (1024×1024)~~ | ~~`app/assets/`~~ | **done** — generated by `tool/make_icon.py`, applied by `flutter_launcher_icons`. Replace the two PNGs to use a designer's artwork | ✅ |
| Feature graphic 1024×500 (Play only) | Play Console | Store listing | ⬜ |
| Six screenshots per language, captured on a device | Both consoles | Store listing (cannot be generated from tests) | ⬜ |
...
| Application id (`com.dollchecker.app` by default) | applied by the script — **decide before the first upload; it is permanent** | Store identity |
| Release signing config | applied by the script with `--release-signing` | Signed release builds |
| Upload keystore itself — **manual** | `android/key.properties` + the `.jks`, or the release workflow's secrets | Any release build |

## 4b. Release pipeline secrets

Set under Settings → Secrets and variables → Actions. Full instructions in
[`docs/RELEASING.md`](RELEASING.md).

| Secret | Purpose | Status |
|---|---|---|
| `ANDROID_KEYSTORE_BASE64` + password, alias, key password | Signing the release bundle | ⬜ |
| `SUPABASE_URL`, `SUPABASE_ANON_KEY` | The release build's `.env` (the committed one holds placeholders) | ⬜ |
| `AUTH_REDIRECT_URL`, `PRIVACY_URL`, `TERMS_URL`, `SUPPORT_EMAIL` | Same | ⬜ |
| `PREMIUM_PRICE_LABEL` | Optional price line | ⬜ |
